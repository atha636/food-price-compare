...
const fetchSwiggyRestaurants = async (lat, lng, food) => {
  try {
    ...
    log.error("Swiggy API error", err);
    return [];
  }
};
...
const fetchZomatoRestaurants = async (lat, lng, food, cityName) => {
  ...
  try {
    ...
    const restaurants = [];
    const list = res.data?.section?.SEARCH_RESULT?.cards || [];
    ...
    if (err.response?.status === 403) {
      log.warn("Zomato blocked (403)");
    } else {
      log.error("Zomato API error", err);
    }
    return [];
  }
};
  ...
  try {
    ...
    if (serviceType === "food") {
      ...
      if (zomatoList.length === 0) {
        log.warn("Zomato empty, using fallback");
    ...
    if (serviceType === "ride") {
      ...
      if (!pickupLat || !pickupLng || !dropLat || !dropLng) {
        log.warn("Ride location not found");
        return res.status(400).json({
