...
const BarTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div style={{
      ...
      <p style={{ color: "#60a5fa", fontWeight: 600, margin: 0, marginBottom: 4 }}>{label}</p>
      <p style={{ color: "#f1f5f9", margin: 0 }}>{payload[0]?.value || 0} {payload[0]?.name || ""}</p>
    </div>
  );
};
...
const PieTooltip = ({ active, payload }) => {
  ...
  return (
    <div style={{
      ...
      <p style={{
        color: payload[0]?.payload?.fill || "#60a5fa",
        fontWeight: 600,
        margin: 0,
        marginBottom: 4,
      ...
      <p style={{ margin: 0 }}>{payload[0]?.value || 0} wins ({payload[0]?.payload?.pct || 0}%)</p>
    </div>
  );
};
...
/* ── Chart Section Card ── */
const ChartCard = ({ title, badge, badgeColor, children, delay = 0 }) => (
  <div style={{
    background: "rgba(255,255,255,0.03)",
  ...
  <div style={{
    ...
    <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
      <div style={{ marginBottom: "40px" }}>
        ...
        <div style={{ width: "220px", height: "36px", background: "#1e293b", borderRadius: "10px", animation: "pulse 1.4s ease 0.1s infinite" }} />
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(120px, 1fr))", gap: "12px", marginBottom: "24px" }}>
        {[0,1,2,3,4].map(i => (
          <div key={i} style={{ height: "100px", borderRadius: "22px", background: "#0f172a", border: "1px solid #1e293b", animation: `pulse 1.4s ease ${i*0.1}s infinite` }} />
        ))}
      </div>
    </div>
  </div>
  ...
  useEffect(() => {
    ...
    const fetchAnalytics = async () => {
      try {
        ...
        const foodCount = {};
        history.forEach(s => {
          if (s?.item) foodCount[s.item] = (foodCount[s.item] || 0) + 1;
        });
        ...
        const platformArr = Object.keys(winCount).map(p => ({
          platform: p?.charAt(0)?.toUpperCase() + p?.slice(1) || "Unknown",
          wins: winCount[p],
        }));
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{
            ...
            <StatCard icon="🔍" label="Total" value={totalSearches} accent="#60a5fa" delay={0.05} />
            <StatCard icon="🍕" label="Top Food" value={topFood?.slice(0, 12)} accent="#f59e0b" delay={0.1} />
            <StatCard icon="🔴" label="Zomato" value={zomatoWins} accent="#f87171" sub={`${zPct}%`} delay={0.15} />
            <StatCard icon="🟠" label="Swiggy" value={swiggyWins} accent="#fb923c" sub={`${sPct}%`} delay={0.2} />
            <StatCard icon="💰" label="Saved" value={`₹${moneySaved}`} accent="#34d399" delay={0.25} />
          </div>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{
                ...
                height: "46px",
                borderRadius: "14px",
                background: `${leaderColor}20`,
                border: `1px solid ${leaderColor}35`,
                display: "flex",
                alignItems: "center",
            ...
            <div style={{
              ...
              <div style={{
                ...
                <span style={{ color: "#f87171" }}>Z {zPct}%</span>
                <span style={{ color: "#fb923c" }}>S {sPct}%</span>
              </div>
              <div style={{
                height: "8px",
                borderRadius: "100px",
          ...
          <div className="chart-grid-2" style={{
            display: "grid",
                ...
                {chartData && chartData.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={chartData} barCategoryGap="40%">
                      <XAxis
                        dataKey="platform"
                        tick={{ fill: "#475569", fontSize: isMobile ? 11 : 12, fontFamily: "'DM Sans',sans-serif" }}
                        axisLine={false}
                        tickLine={false}
                        angle={isMobile ? -45 : 0}
                    ...
                    <div style={{
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          ...
                          >
                            {pieData.map((entry, i) => (
                              <Cell key={i} fill={entry?.fill || "#60a5fa"} stroke="transparent" />
                            ))}
                          </Pie>
          ...
          <div className="chart-grid-2" style={{
            ...
            {/* Food Bar */}
            <ChartCard title="🍕 Top Foods" delay={0.45}>
              <div style={{
                flex: 1,
                width: "100%",
                minHeight: isMobile ? "200px" : "240px",
                ...
                {foodChart && foodChart.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={foodChart} barCategoryGap="35%">
                      <XAxis
                        dataKey="food"
                        tick={{ fill: "#475569", fontSize: isMobile ? 10 : 11, fontFamily: "'DM Sans',sans-serif" }}
                        axisLine={false}
                        tickLine={false}
                        angle={isMobile ? -45 : 0}
                        textAnchor={isMobile ? "end" : "middle"}
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={trendData}>
                    ...
                    <Line
                      ...
                      strokeWidth={2.5}
                      dot={{ fill: "#60a5fa", r: isMobile ? 3 : 4, strokeWidth: 0 }}
                      activeDot={{ r: isMobile ? 4 : 6, fill: "#34d399", strokeWidth: 0 }}
